...
def build_ydl_options(temp_dir, log_capture, url, use_cookies=True, instagram_format=None):
    ...
    ydl_opts = {
        'noplaylist': True,
        'format': instagram_format or 'bv*+ba/b',
...
def _download_once(url, temp_dir, log_capture, use_cookies=True, instagram_format=None):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if is_instagram(url):
            has_audio = _print_format_debug(info)
            if not has_audio:
                return None, info
...
def download_and_merge(url, temp_dir):
    ...
    try:
        # Instagram: first try with cookies. If Instagram exposes only
        # video-only formats, retry once without cookies. This is useful
        # for public reels where a logged-in cookie jar can sometimes
        # change the formats returned by Instagram.
        if is_instagram(url):
            info = None

            # First try Instagram's best single-file format.
            # Some music Reels expose a muxed video+audio stream while
            # the normal bv*+ba selector cannot pair the streams.
            for fmt in ('best', 'bv*+ba/b'):
                try:
                    log_capture.lines.append(
                        'Instagram attempt with cookies, format=' + fmt
                    )
                    ...
                    log_capture.lines.append(
                        'Instagram cookie attempt failed (' + fmt + '): ' + str(first_error)
                    )
                    info = None
            ...
            if info is None:
                # Clean partial files before the fallback attempt.
                for f in glob.glob(os.path.join(temp_dir, '*')):
                    try:
                        if os.path.isfile(f):
                            os.remove(f)
                ...
                log_capture.lines.append(
                    'Instagram fallback: retrying without cookies.'
                )

                for fmt in ('best', 'bv*+ba/b'):
                    try:
                        log_capture.lines.append(
                            'Instagram logged-out attempt, format=' + fmt
                        )
                        info, extracted = _download_once(
                            url,
                            temp_dir,
                        ...
                        log_capture.lines.append(
                            'Instagram logged-out attempt failed (' + fmt + '): ' + str(second_error)
                        )
                        info = None
            ...
            if info is None:
                return None, None, {
                    'error': (
                        'Instagram did not provide an audio stream. '
                        'The post may currently expose video-only media to yt-dlp.'
                    ),
                    'logs': log_capture.lines[-60:]
                }
