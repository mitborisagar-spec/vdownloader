...
# Make sure yt-dlp can find the Deno runtime we installed via the custom
# Render build command (DENO_INSTALL=/opt/render/project/.deno). Deno is
# needed to solve YouTube's "n" signature challenge; without it, YouTube
# formats come back scrambled/unusable even with valid cookies + PO tokens.
...
def extract_video_url(url):
    ...
    ydl_opts = {'noplaylist': True, 'logger': log_capture, 'verbose': True}

    if is_youtube(url):
        cookies_path = get_writable_cookies_path()
        if cookies_path:
            ydl_opts['cookiefile'] = cookies_path
        # Point yt-dlp at our deployed PO Token provider server. Different
        # plugin versions use different extractor_args key conventions, so
        # we set both to maximize the chance one of them is picked up.
        ydl_opts['extractor_args'] = {
            'youtubepot-bgutilhttp': {
                'base_url': ['https://vdownloader-pot.onrender.com']
            },
            'youtube': {
                'getpot_bgutil_baseurl': ['https://vdownloader-pot.onrender.com']
            }
        }

    def relevant_logs():
        return [
            line for line in log_capture.lines
            if 'pot' in line.lower() or 'token' in line.lower() or 'provider' in line.lower()
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if not video_url:
                debug = {
                    ...
                    'pot_related_logs': relevant_logs(),
                    'sample': [
                        {
        ...
        # Even when yt-dlp raises outright (e.g. "page needs to be
        # reloaded"), still surface the captured PO-token-related debug
        # lines so we're not debugging blind.
        debug = {
            'deno_found': shutil.which('deno') is not None,
            'deno_dir_exists': os.path.isdir(_deno_bin),
            'exception': str(e),
